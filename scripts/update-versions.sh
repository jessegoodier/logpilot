...
# Chart files
VERSION_FILES["charts/logpilot/Chart.yaml"]="s/^version:.*/version: $NEW_VERSION/; s/^appVersion:.*/appVersion: \"$NEW_VERSION\"/"
VERSION_FILES["charts/logpilot/pyproject.toml"]="s/^version = .*/version = \"$NEW_VERSION\"/"
VERSION_FILES["charts/logpilot/src/__init__.py"]="s/^__version__ = .*/__version__ = \"$NEW_VERSION\"/"
...
echo "Chart version:"
grep -E "^(version|appVersion):" charts/logpilot/Chart.yaml || echo "Chart.yaml not found"
echo ""
echo "Python package versions:"
grep "^version = " pyproject.toml || echo "Root pyproject.toml not found"
grep "^version = " charts/logpilot/pyproject.toml || echo "Chart pyproject.toml not found"
echo ""
echo "Python module versions:"
grep "^__version__" src/__init__.py || echo "Root __init__.py not found"
grep "^__version__" charts/logpilot/src/__init__.py || echo "Chart __init__.py not found"
echo ""
