...
def update_bumpversion_config(current_version: str) -> None:
    ...
    new_content = re.sub(r'current_version = "[^"]*"', f'current_version = "{current_version}"', content)
    config_path.write_text(new_content)


def get_target_version(version_bump: str, custom_version: Optional[str] = None, version_type: str = "dev") -> str:
    ...
    if version_bump == "custom":
        ...
        # Apply version_type transformation to custom version
        import re

        if version_type == "release":
            # Remove -dev suffix if present
            return re.sub(r"-dev$", "", custom_version)
        elif version_type == "dev":
            # Add -dev suffix if not present
            if not custom_version.endswith("-dev"):
                return f"{custom_version}-dev"
    ...
    current_version_cmd = subprocess.run(
        ["bump-my-version", "bump", "--allow-dirty", "--dry-run", "release"], capture_output=True, text=True
    )
    ...
    # Apply version_type transformation
    if version_type == "release":
        # Remove -dev suffix if present
        target_version = re.sub(r"-dev$", "", target_version)
    elif version_type == "dev":
        # Add -dev suffix if not present
        if not target_version.endswith("-dev"):
            target_version = f"{target_version}-dev"
...
def main():
    ...
    version_type = os.environ.get("INPUT_VERSION_TYPE", "dev")
