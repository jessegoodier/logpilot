...
def test_log_viewer_e2e(page: Page):
    ...
    try:
        ...
        log_content_after_wait = log_output.inner_html() # Use inner_html to check for styled content too
        ...
        for i in range(log_lines.count()):
            ...
            # Check if the message span *text content* contains the search term,
            # for cases where highlighting might be complex or cover only part of the word in some edge cases.
            message_text_content = message_span.text_content() or ""
        ...
        screenshot_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "test-results", "test-failure-screenshot.png")
        page.screenshot(path=screenshot_path)
        print(f"Test failed: {e}")
        pytest.fail(f"Test failed due to: {e}")

# To run this test:
# 1. Ensure the Flask app (app.py) is running: `pipenv run gunicorn app:app -b 0.0.0.0:5001` or `pipenv run python app.py`
# 2. Ensure the log-gen-deployment is active in your K8s cluster and app.py can connect to it.
# 3. Run pytest: `pipenv run pytest tests/test_app_playwright.py --headed` (for headed mode) or `pipenv run pytest tests/test_app_playwright.py` (for headless)